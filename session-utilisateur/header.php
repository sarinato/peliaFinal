<div class="menu-trigger">
		<span></span>
		<span></span>
		<span></span>
	</div>
<header class="fixed-menu">
		<span class="menu-close"><i class="fa fa-times"></i></span>
		<a href="profil.php">
		<div class="menu-header">
			<div class="logo d-flex justify-content-center">
				<h1 class="logo-g">Pélia</h1>
				<!-- <img src="img/logo.png" alt=""> -->
			</div>
		</div></a>
		<div class="nav-wraper">
			<div class="navbar">
				<ul class="navbar-nav">
					<!-- classe d'active page: active -->
					<li class="nav-item"><a class="nav-link" href="profil.php"><img src="img/header/home.png" alt=""> home</a></li>
					<li class="nav-item"><a class="nav-link" href="profile.php"><img src="img/User.svg" alt=""> Profile</a></li>

					<li class="nav-item submenu dropdown">
						<a class="nav-link dropdown-toggle" href="#" data-toggle="dropdown" role="button" aria-haspopup="true"
						 aria-expanded="false"><img src="img/header/pilule.png" alt="">Medicament</a>
						<ul class="dropdown-menu">
							<li class="nav-item"><a class="nav-link" href="medicament.php">Liste medicament</a></li>
							<li class="nav-item"><a class="nav-link" href="ajoutMedicament.php">Ajouter medicament</a></li>

						</ul>
					</li>
					<li class="nav-item submenu dropdown">
						<a class="nav-link dropdown-toggle" href="#"data-toggle="dropdown" role="button" aria-haspopup="true"
						 aria-expanded="false"><img src="img/header/doctor.png" alt="">Medecin</a>
						 <ul class="dropdown-menu">
							<li class="nav-item"><a class="nav-link" href="ListeMedecin.php">Liste médecin</a></li>
							<li class="nav-item"><a class="nav-link" href="medecin.php">Ajouter médecin</a></li>

						</ul>
						</li>
					<li class="nav-item submenu dropdown">
						<a class="nav-link dropdown-toggle" href="rendez-vous.php" data-toggle="dropdown" role="button" aria-haspopup="true"
						 aria-expanded="false"><img src="img/header/date.png" alt="">Rendez vous</a>
						 <ul class="dropdown-menu">
							<li class="nav-item"><a class="nav-link" href="ListeRendez-vous.php">Liste Rendez-vous</a></li>
							<li class="nav-item"><a class="nav-link" href="rendez-vous.php">Ajouter Rendez-vous</a></li>

						</ul>
						</li>
					<li class="nav-item"><a class="nav-link" href="about-us.php"><img src="img/header/about.png" alt=""><span style="position:relative;right:2%;font-size:11px;">Qui sommes-nous</span></a></li>
					<li class="nav-item"><a class="nav-link" href="faq.php"><img src="img/header/faq.png" alt="">FAQ</a></li>
					<li class="nav-item"><a class="nav-link" href="contact.php"><img src="img/header/contact.png" alt="">Contact</a></li>

					<li class="nav-item"><a href="logout.php" class="nav-link"><img src="img/header/logout.svg" alt="">Déconnecter</a>
					
			</div>
		</div>
	</header>